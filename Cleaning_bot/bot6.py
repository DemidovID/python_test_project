...
async def service_info(update: Update, context):
    ...
    await query.edit_message_text(service_map.get(query.data, "Неизвестная команда"))
...
async def choose_service(update: Update, context):
    ...
    context.user_data['service'] = service_map.get(query.data, 'Стандартная')
...
async def enter_address(update: Update, context):
    ...
    await update.message.reply_text("Введите дату и время (например, 25.10.2023 14:00):")
